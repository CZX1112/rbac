...
if docker volume inspect rbac-maven-repo &> /dev/null; then
    echo -e "${GREEN}Maven 缓存 volume 已存在。${NC}"
else
    echo -e "${GREEN}创建 Maven 缓存 volume...${NC}"
    docker volume create --name rbac-maven-repo
fi
...
docker run -it --rm --name rbac-maven \
    -v rbac-maven-repo:/root/.m2 \
    -v "$PWD/rbac-server":/usr/src/mymaven \
    -w /usr/src/mymaven \
...
if docker-compose ps | grep "rbac"; then
